...
lint-go-golangci:
	$(info ==> $@)
	@tools/quiet bazel run --config=quiet @rules_go//go -- run github.com/golangci/golangci-lint/v2/cmd/golangci-lint@v2.9.0 run --config="${PWD}/.golangci.yml" $(GO_BUILD_TAGS_ARG),lint ./...
